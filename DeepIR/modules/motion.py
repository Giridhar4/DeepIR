...
def get_SR_data(im, im_t, scale, nimg, config=None): # im_t is a thermal image
    '''
    ...
    if simulation is True:
        H, W = im.shape
        Ht, Wt = im_t.shape
        imstack, imstack_t, _, _, _, _, mats, t_mats = get_imstack(im, im_t, scale, shift_max,
                                          theta_max, nimg)  # added thermal img
        imstack /= im.max()
        im /= im.max()
        ...
        _, Htl, Wtl = imstack_t.shape
        if get_gt:
            ecc_mats = invert_regstack(mats)
        else:
            ecc_mats = register_stack(imstack, (Hl, Wl))[:, :, :] # if you are getting an error insert :2
            ecc_t_mats = register_stack_t(imstack_t, (Htl, Wtl))[:, :, :]
    else:
        _, H, W = im.shape
    ...
    return im, imstack, imstack_t, ecc_mats, ecc_t_mats
...
def get_imstack(im, im_t, scale, shift_max=10, theta_max=np.pi / 12, nshifts=5):
    '''
    ...
    Xstack, Xstack_t = np.zeros_like(imstack), np.zeros_like(imstack_t)
    Ystack, Ystack_t = np.zeros_like(imstack), np.zeros_like(imstack_t)
    ...
    for idx in range(nshifts):
        ...
        Xstack_t[idx, ...] = 2 * Xtnew / Wt -1
        Ystack_t[idx, ...] = 2 * Ytnew / Ht -1
    return imstack, imstack_t, Xstack, Ystack, Xstack_t, Ystack_t, mats, t_mats
...
def register_stack_t(imstack_t, full_res, method=StackReg.RIGID_BODY):   ### registration for thermal
    '''
        Register a stack of images and get coordinates
        ...
        Outputs:
            reg_mats: (nimg, 2, 3) dimensional registration matrices
    '''
    nimg, H, W = imstack_t.shape
    Hr, Wr = full_res

    imstack_t_full = np.zeros((nimg, Hr, Wr))

    # Upsample the images
    for idx in range(nimg):
        imstack_t_full[idx, ...] = cv2.resize(imstack_t[idx, ...], (Wr, Hr),
                                            interpolation=cv2.INTER_AREA)

    # Now register the stack
    reg = StackReg(method)
    reg_mats = reg.register_stack(imstack_t_full, reference='first', verbose=True)
