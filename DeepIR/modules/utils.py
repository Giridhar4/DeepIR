...
def get_img(imname, scaling):
    ...
    # im_t = cv2.resize(imname_thermal, None, fx=scaling, fy=scaling)
    # im_t = np.copy(im_t, order='C')
    if im.ndim == 2:
        im = im[:, :, np.newaxis]
        im = im[:, :, [0, 0, 0]]
...
def get_img_t(imname_thermal, scaling):
    # Read image
    im = cv2.resize(plt.imread('data/%s.png' % imname_thermal), None,
                    fx=scaling, fy=scaling)
